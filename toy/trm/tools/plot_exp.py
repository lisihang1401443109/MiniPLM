...
plot, ax = plt.subplots(1, 1, figsize=(8, 3))
...
for i, path in enumerate(paths):
    ...
    plt.scatter(steps[min_steps:], loss[min_steps:]-bias, s=60, color="lightblue" if i == 0 else "lightcoral", label="Conventional LM Learning    (Experiment)" if i == 0 else "Near-Optimal LM Learning    (Experiment)")
    ...
    label_str = r"$\text{Loss} =\left(" + f"{T_min_2:.2f}" + r"\times 10^" + f"{T_min_1}" + r"/t\right)^{" + f"{-a:.2f}" + r"}" + r"$ (Extrapolated)"
    
    # ax.plot(steps[min_steps:], np.exp(f(log_steps[min_steps:], *popt)), "--", label=label_str, linewidth=3, color="blue" if i == 0 else "darkred")

    max_steps = int(1e7) if i == 0 else int(1e7 / 9.232495716315531)
    
    full_steps = np.array(range(0, max_steps, 10000))
    full_steps[0] = min_steps
    ax.plot(full_steps, np.exp(f(np.log(full_steps), *popt)), "--", label=label_str, linewidth=3, color="blue" if i == 0 else "red", alpha=0.5 if i == 0 else 1)
...
ax.set_yticks([2.0, 3.0, 4.0], [2.0, 3.0, 4.0])
ax.tick_params(axis='both', which='both', labelsize=18)
plt.legend(fontsize=18, bbox_to_anchor=(1.01, 1.85))
# plt.title("Transformer Language Modeling", fontsize=18)
plt.savefig("/home/lidong1/yuxian/sps-toy/toy/trm/tools/exp.png", bbox_inches="tight", dpi=300)
