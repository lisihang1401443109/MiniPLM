...
for i in steps:
    if i == -1:
        test_losses = torch.load(os.path.join(base_path, f"baseline/test_loss.pt"), map_location="cpu")
    else:
        test_losses = torch.load(os.path.join(base_path, f"opt_0.001_epoch_{i}/test_loss.pt"), map_location="cpu")
...
ax1.set_xlabel(r"Training Epochs of $\gamma$-OptNet", fontsize=14)
